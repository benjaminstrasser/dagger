...
import { execaCommand, ExecaChildProcess } from "npm:execa@8.0.1"
import { GraphQLClient } from "npm:graphql-request@6.1.0"
import fetch from "npm:node-fetch@3.3.2"
import * as tar from "npm:tar@6.2.0"
  ...
  private async runEngineSession(
    ...
    const connectParams: ConnectParams = (await Promise.race([
      ...
      new Promise((_, reject) => {
        ...
        }, timeOutDuration) // long timeout to account for extensions, though that should be optimized in future
      }),
    ])) as ConnectParams
